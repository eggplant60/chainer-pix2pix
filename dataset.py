...
class FacadeDataset(dataset_mixin.DatasetMixin):
    def __init__(self, dataDir='./azuren', data_range=(1,2000)):
        ...
        for i in range(data_range[0],data_range[1]):
            ...
            # TODO: すべてメモリに読み込むとあふれるので逐次読み出しにする
            img = Image.open(dataDir+"/cmp_c%04d.jpg"%i)
            label = Image.open(dataDir+"/cmp_l%04d.jpg"%i)
            w,h = img.size
            r = 286 / float(min(w,h))
